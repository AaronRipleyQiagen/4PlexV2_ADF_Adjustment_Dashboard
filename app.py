...
def check_cutoffs(data: pd.DataFrame, settings: dict):
    target_result_frame = data.copy()
    target_result_frame["Simulated Target Result"] = np.where(
        (
            (
                target_result_frame["Max Peak Height"].round(2)
                > settings["minimum-peak-threshold"]
            )
            & (
                target_result_frame["End Point Fluorescence"].round(2)
                > settings["minimum-ep-threshold"]
            )
            & (
                target_result_frame["Ct"].round(2)
                <= (settings["maximum-peak-cycle-threshold"]) + 0.5
            )
            & (
                target_result_frame["EPR"].round(2)
                >= settings["overall-epr-check-threshold"]
            )
            & (
                (target_result_frame["Ct"] > settings["epr-check-ct-threshold"])
                | (
                    (target_result_frame["Ct"] <= settings["epr-check-ct-threshold"])
                    & (
                        target_result_frame["EPR"].round(2)
                        > settings["epr-check-threshold"]
                    )
                )
            )
        ),
...
def classify_result(
    ...
    observed_column="Simulated Target Result",
    pos_result="POS",
    neg_result="NEG",
):
    """
    ...
    classify_dataframe["Classification"] = np.where(
        (
            (classify_dataframe[expected_column] == pos_result)
    ...
    classify_dataframe["Classification"] = np.where(
        (
            (classify_dataframe[expected_column] == neg_result)
            & (classify_dataframe[observed_column] == pos_result)
        ),
        "FP",
        classify_dataframe["Classification"],
...
def build_classification_array(
    data: pd.DataFrame, classification_column="Classification"
):
    """
    ...
    classify_array_dataframe["TP"] = np.where(
        classify_array_dataframe[classification_column] == "TP", 1, 0
    )
    classify_array_dataframe["FP"] = np.where(
        classify_array_dataframe[classification_column] == "FP", 1, 0
    )
    classify_array_dataframe["TN"] = np.where(
        classify_array_dataframe[classification_column] == "TN", 1, 0
    )
    classify_array_dataframe["FN"] = np.where(
        classify_array_dataframe[classification_column] == "FN", 1, 0
    )
...
def calculate_sensitivity_specificity(
    data: pd.DataFrame, TP_column="TP", FP_column="FP", TN_column="TN", FN_column="FN"
):
    sensitivity_specificity_dataframe = data.copy()

    sensitivity_specificity_dataframe_grouped = sensitivity_specificity_dataframe[
        [TP_column, FP_column, TN_column, FN_column]
    ].agg("sum")

    sensitivity = sensitivity_specificity_dataframe_grouped[TP_column] / (
        sensitivity_specificity_dataframe_grouped[TP_column]
        + sensitivity_specificity_dataframe_grouped[FN_column]
    )
    specificity = sensitivity_specificity_dataframe_grouped[TN_column] / (
        sensitivity_specificity_dataframe_grouped[TN_column]
        + sensitivity_specificity_dataframe_grouped[FP_column]
    )

    return sensitivity, specificity


def calculate_simulated_sensitivity_specificity(data: pd.DataFrame, settings: dict):
    simulated_frame = data.copy()
    simulated_frame["Simulated Target Result"] = check_cutoffs(
        simulated_frame, settings
    )
    simulated_frame["Classification"] = classify_result(simulated_frame)
    simulated_frame[["TP", "FP", "TN", "FN"]] = build_classification_array(
        simulated_frame
    )
...
def calculate_current_sensitivity_specificity(data: pd.DataFrame):
    ...
    current_frame["Classification"] = classify_result(
        current_frame, observed_column="Reported Result"
    )
...
def serve_layout():
    ...
    valid_ct_window_adjustment = dcc.RangeSlider(
        0, 50, 1, value=[11, 37], marks=ct_range_marks, id="ct-window-threshold"
    )
    ...
    min_ep_cutoff = dcc.Slider(
        0, 5000, 100, value=1200, marks=ep_marks, included=False, id="min-ep-threshold"
    )
    ...
    min_peak_cutoff = dcc.Slider(
        75,
        150,
        1,
        value=75,
        marks=min_peak_marks,
        included=False,
        id="min-peak-threshold",
    )
    ...
    """
    epr_check_ct_threshold_label = html.Label(
        "Set EPR Ct Check Threshold", style=label_style
    )
    ...
    epr_check_ct_threshold_cutoff = dcc.Slider(
        20,
        40,
        1,
        value=30,
        marks=epr_check_ct_threshold_marks,
        included=False,
        id="epr-ct-check-threshold",
    )
    epr_check_ct_threshold_cutoff_value = html.Label(
        id="epr-ct-check-threshold-cutoff-value", style=label_style
    )
    ...
    epr_threshold_marks = {}
    for mark in range(100, 205, 5):
        epr_threshold_marks[(mark / 100)] = {"label": str(round((mark / 100), 2))}

    epr_threshold_cutoff = dcc.Slider(
        1,
        2.05,
        0.01,
        marks=epr_threshold_marks,
        value=1.15,
        included=False,
        id="epr-threshold",
    )
    ...
    """
    overall_epr_threshold_label = html.Label(
        "Set Overall EPR Check Threshold", style=label_style
    )

    overall_epr_threshold_marks = {}
    for mark in range(100, 150, 5):
        overall_epr_threshold_marks[(mark / 100)] = {
            "label": str(round((mark / 100), 2))
        }

    overall_epr_threshold_cutoff = dcc.Slider(
        1,
        1.50,
        0.01,
        marks=overall_epr_threshold_marks,
        value=1.05,
        included=False,
        id="overall-epr-threshold",
    )
    overall_epr_threshold_value = html.Label(
        id="overall-epr-threshold-value", style=label_style
    )
    ...
    clinical_sensitivity = daq.Gauge(
        color={
            "default": "black",
            "gradient": True,
            "ranges": {"red": [0, 85], "yellow": [85, 95], "green": [95, 100]},
        },
    ...
    sample_summary_barchart = dcc.Graph(id="sample-summary-barchart")

    """
    Assemble the Card Body for Summary Results
    """

    summary_content = dcc.Loading(
        id="updating-graphs",
        type="graph",
        children=[
            dbc.Card(
    ...
    affected_samples_content = dcc.Loading(
        id="updating-tables",
        type="dot",
        children=[dbc.Card(dbc.CardBody([affected_samples]))],
    )
    ...
    """
    clincial_affected_samples_data = dcc.Store(
        id="clinical-affected-samples-data", storage_type="session", clear_data=True
    )
    analytical_affected_samples_data = dcc.Store(
        id="analytical-affected-samples-data", storage_type="session", clear_data=True
    )
    customer_fps_affected_samples_data = dcc.Store(
        id="customer-fps-samples-data", storage_type="session", clear_data=True
    )
    ...
    return html.Div(
        children=[
            settings,
            dcc.Loading(
                id="uploading", children=[uploaded_data], type="dot", fullscreen=True
            ),
            ...
            html.Div(
                children=[
                    ...
                    html.Div(
                        children=[
                            html.H3("Select Channel / Target of Interest"),
                            channel_selector,
                        ],
                        style={
                            "width": "50%",
            ...
            html.Div(
                children=[
                    html.Div(
                        children=[html.H3("Set ADF Parameter Settings")],
                        style={"padding": "10px"},
                    ),
                    html.Div(
                        children=[
                            ct_range_label,
                            ct_range_values,
                            valid_ct_window_adjustment,
                        ],
                        style=cutoff_selection_style,
                    ),
                    html.Div(
                        children=[min_ep_label, min_ep_cutoff_value, min_ep_cutoff],
                        style=cutoff_selection_style,
                    ),
                    html.Div(
                        children=[
                            min_peak_label,
                            min_peak_cutoff_value,
                            min_peak_cutoff,
                        ],
                        style=cutoff_selection_style,
                    ),
                    html.Div(
                        children=[
                            overall_epr_threshold_label,
                            overall_epr_threshold_value,
                            overall_epr_threshold_cutoff,
                        ],
                        style=cutoff_selection_style,
                    ),
                    html.Div(
                        children=[
                            epr_check_ct_threshold_label,
                            epr_check_ct_threshold_cutoff_value,
                            epr_check_ct_threshold_cutoff,
                        ],
                        style=cutoff_selection_style,
                    ),
                    html.Div(
                        children=[
                            epr_threshold_label,
                            epr_threshold_value,
                            epr_threshold_cutoff,
                        ],
                        style=cutoff_selection_style,
                    ),
                    html.Div(
                        children=[
                            specimen_type_selection_label,
                            specimen_type_selection,
                        ],
                        style=cutoff_selection_style,
                    ),
                ],
...
@dash_app.callback(
    [Output("uploaded-data", "data"), Output("uploaded-data-msg", "children")],
...
def store_uploaded_data(contents):
    if contents is not None:
        ...
        dashboard_data["Reported Result"] = dashboard_data["Localized Result"].replace(
            {"TargetNotAmplified": "NEG", "TargetAmplified": "POS"}
        )
        dashboard_data_valid = dashboard_data[
            dashboard_data["Reported Result"].isin(["POS", "NEG"])
        ]
        dashboard_data_valid = dashboard_data_valid[
            ~dashboard_data_valid["Expected Result"].isin(["Exclude", "LPOS"])
        ]
        dashboard_data_valid["Expected Result"] = np.where(
            dashboard_data_valid["Expected Result"] == "NEG",
            "NEG",
            "POS",
        )
        data_dict = dashboard_data_valid.to_dict("records")
        return data_dict, "Successfully uploaded data for {} valid samples".format(
            str(len(dashboard_data_valid))
        )
    else:
...
@dash_app.callback(
    Output("specimen-type-selection", "options"),
...
def get_settings(
    ct_window, min_ep, min_peak, overall_epr, epr_ct_check, epr, specimen_type, channel
):
    if ct_window:
        settings = {
            "minimum-peak-cyle-threshold": ct_window[0],
...
@dash_app.callback(
    [
        Output("clinical-sensitivity", "value"),
        ...
        Output("sample-summary-barchart", "figure"),
    ],
    [Input("settings", "data"), State("uploaded-data", "data")],
    prevent_initial_call=True,
)
def update_sensitivity_specificity_fps_kpis(settings, uploaded_data):
    if uploaded_data:
        dataframe = pd.DataFrame.from_dict(uploaded_data)
        dataframe["Sample Count"] = 1
        if settings["specimen-type"]:
            if settings["specimen-type"] != "All":
                dataframe = dataframe[
                    dataframe["Target Setting Specimen Type"]
                    == settings["specimen-type"]
                ]
        dataframe = dataframe[dataframe["Channel"] == settings["channel"]]
        ...
        (
            original_clinical_sensitivity,
            original_clinical_specificity,
        ) = calculate_current_sensitivity_specificity(dataframe_clincial)
        (
            original_analytical_sensitivity,
            original_analytical_specificity,
        ) = calculate_current_sensitivity_specificity(dataframe_analytical)
        (
            clinical_sensitivity,
            clinical_specificity,
        ), dataframe_clinical = calculate_simulated_sensitivity_specificity(
            dataframe_clincial, settings
        )
        (
            analytical_sensitivity,
            analytical_specificity,
        ), dataframe_analytical = calculate_simulated_sensitivity_specificity(
            dataframe_analytical, settings
        )

        dataframe_clinical_changes = dataframe_clinical[
            dataframe_clinical["Simulated Target Result"]
            != dataframe_clinical["Reported Result"]
        ]
        dataframe_analytical_changes = dataframe_analytical[
            dataframe_analytical["Simulated Target Result"]
            != dataframe_analytical["Reported Result"]
        ]

        dataframe_customer = dataframe[dataframe["Data Source"] == "Customer"]
        dataframe_customer["Simulated Target Result"] = check_cutoffs(
            dataframe_customer, settings
        )
        dataframe_customer_changes = dataframe_customer[
            dataframe_customer["Simulated Target Result"]
        ...
        return (
            clinical_sensitivity * 100,
            ((clinical_sensitivity - original_clinical_sensitivity) * 100).round(2),
            clinical_specificity * 100,
            ((clinical_specificity - original_clinical_specificity) * 100).round(2),
            analytical_sensitivity * 100,
            ((analytical_sensitivity - original_analytical_sensitivity) * 100).round(2),
            analytical_specificity * 100,
            ((analytical_specificity - original_analytical_specificity) * 100).round(2),
            len(dataframe_customer),
            len(
                dataframe_customer[
                    dataframe_customer["Simulated Target Result"] != "NEG"
                ]
            ),
            len(
                dataframe_customer[
            ...
            - len(dataframe_customer),
            dataframe_clinical_changes.to_dict("records"),
            dataframe_analytical_changes.to_dict("records"),
            dataframe_customer_changes.to_dict("records"),
            fig,
        )
    else:
        return dash.no_update
...
@dash_app.callback(
    Output("clinical-sensitivity-impact", "color"),
    Input("clinical-sensitivity-impact", "value"),
    prevent_initial_call=True,
)
...
@dash_app.callback(
    Output("clinical-specificity-impact", "color"),
    Input("clinical-specificity-impact", "value"),
    prevent_initial_call=True,
)
def update_clinical_sensitivity_color(value):
    if value < 0:
...
@dash_app.callback(
    Output("analytical-sensitivity-impact", "color"),
    Input("analytical-sensitivity-impact", "value"),
    prevent_initial_call=True,
)
def update_clinical_sensitivity_color(value):
    if value < 0:
...
@dash_app.callback(
    Output("analytical-specificity-impact", "color"),
    Input("analytical-specificity-impact", "value"),
    prevent_initial_call=True,
)
def update_clinical_sensitivity_color(value):
    if value < 0:
...
@dash_app.callback(
    Output("customer-fps-impact", "color"),
    Input("customer-fps-impact", "value"),
    prevent_initial_call=True,
)
def update_clinical_sensitivity_color(value):
    if value <= 0:
...
@dash_app.callback(
    [
        Output("affected-samples-table", "rowData"),
...
def get_affected_sample_results(clinical_samples, analytical_samples, customer_fps):
    ...
    all_affected_samples = pd.concat(
        [
            clinical_samples_dataframe,
            analytical_samples_dataframe,
            customer_fps_dataframe,
        ]
    )
    visable_columns = [
        "Data Source",
    ...
    for column in all_affected_samples.columns:
        if (
            column == "Data Source"
            or column == "Protocol"
            or column == "Target Setting Specimen Type"
        ):
...
@dash_app.callback(
    Output("min-ep-cutoff-value", "children"), [Input("min-ep-threshold", "value")]
)
...
@dash_app.callback(
    Output("min-peak-cutoff-value", "children"), [Input("min-peak-threshold", "value")]
)
def update_ep_cutoff_label(value):
    return str(value)
...
@dash_app.callback(
    Output("epr-ct-check-threshold-cutoff-value", "children"),
    [Input("epr-ct-check-threshold", "value")],
)
def update_ep_cutoff_label(value):
    return str(value)
...
@dash_app.callback(
    Output("epr-threshold-value", "children"), [Input("epr-threshold", "value")]
)
def update_ep_cutoff_label(value):
    return str(value)
...
@dash_app.callback(
    Output("overall-epr-threshold-value", "children"),
    [Input("overall-epr-threshold", "value")],
)
def update_ep_cutoff_label(value):
    return str(value)
...
@dash_app.callback(
    Output("ct-range-values", "children"), [Input("ct-window-threshold", "value")]
)
def update_ep_cutoff_label(value):
    print(value[0])
